...
async def fetch_dgcmef_quotidien(source: dict, run_id: str) -> dict:
    ...
    logger.info(
        "Fetching DGCMEF quotidien", source=source_name, url=list_url, run_id=run_id
    )
    ...
    try:
        ...
        if len(cells) < 2:
            raise Exception(
                f"Invalid table structure: found {len(cells)} columns, expected at least 2"
            )
...
async def download_quotidien_pdf(pdf_url: str, source_name: str, run_id: str) -> dict:
    ...
    logger.info(
        "Downloading quotidien PDF", source=source_name, pdf_url=pdf_url, run_id=run_id
    )
