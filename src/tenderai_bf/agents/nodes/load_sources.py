...
def load_sources_node(state) -> dict:
    ...
    try:
        ...
        logger.info(
            "Syncing sources with database (production mode)", run_id=state.run_id
        )
            ...
            for config_source in config_sources:
                ...
                try:
                    ...
                    if not db_source:
                        ...
                        db_source.base_url = config_source.get(
                            "base_url", db_source.base_url
                        )
                        db_source.list_url = config_source.get(
                            "list_url", db_source.list_url
                        )
                        db_source.parser_type = config_source.get(
                            "parser", db_source.parser_type
                        )
                        db_source.rate_limit = config_source.get(
                            "rate_limit", db_source.rate_limit
                        )
                        db_source.enabled = config_source.get(
                    ...
                    state.add_error(
                        "load_sources",
                        f"Failed to process source {source_name}: {e!s}",
                        source_name=source_name,
                    )
