...
def parse_french_date(date_str: str) -> datetime | None:
    ...
    try:
        ...
        logger.debug("Failed to parse French date", date_str=date_str, error=str(e))
        return None


def parse_deadline(
    deadline_str: str, reference_date: datetime | None = None
) -> datetime | None:
    ...
    try:
        ...
        logger.debug(
            "Failed to parse deadline", deadline_str=deadline_str, error=str(e)
        )
        return None
...
def format_deadline(dt: datetime, reference_date: datetime | None = None) -> str:
    ...
    if days_diff == 0:
        ...
        if days_diff <= 7:
            ...
            weeks = days_diff // 7
            return (
                f"{formatted_date} (il y a {weeks} semaine{'s' if weeks > 1 else ''})"
            )
        else:
            months = days_diff // 30
...
def get_next_business_day(dt: datetime | None = None) -> datetime:
...
def time_until_deadline(
    deadline: datetime, reference_date: datetime | None = None
) -> timedelta:
