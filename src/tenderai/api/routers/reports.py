...
async def list_reports(
    db: DatabaseSession, current_user: AuthenticatedUser, limit: int = 50
):
...
async def download_report(run_id: str, db: DatabaseSession):
    ...
    try:
        ...
        logger.error("Failed to download report", run_id=run_id, error=str(e))
...
async def regenerate_report(run_id: str, db: DatabaseSession):
    ...
    try:
        ...
        report_url = storage_client.store_report(
            report_data=report_bytes, run_id=str(run.id), timestamp=datetime.utcnow()
        )
        ...
        logger.info(
            "Report regenerated successfully", run_id=run_id, report_url=report_url
        )
        ...
        logger.error("Failed to regenerate report", run_id=run_id, error=str(e))
