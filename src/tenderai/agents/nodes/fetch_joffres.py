...
def extract_joffres_listings(html_content: str, source_url: str) -> list[dict]:
    ...
    try:
        ...
        logger.info(f"Found {len(tender_links)} tender listings", source_url=source_url)
...
def extract_joffres_detail(html_content: str, detail_url: str) -> dict:
    ...
    try:
        ...
        deadline = ""
        for strong_elem in parser.css(
            ".offre-detail-right strong, .offre-detail-ight strong"
        ):
        ...
        logger.debug(
            "Extracted detail page", title=title[:50], ref=ref_no, entity=entity
        )
