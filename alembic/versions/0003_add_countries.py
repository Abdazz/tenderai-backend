...
def upgrade() -> None:
    ...
    if _table_exists(bind, "sources"):
        ...
        # Always backfill — covers the case where create_all() added the column without seeding.
        op.execute(
            "UPDATE sources SET country_id = (SELECT id FROM countries WHERE code = 'BF') "
    ...
    if _table_exists(bind, "runs"):
        ...
        op.execute(
            "UPDATE runs SET country_id = (SELECT id FROM countries WHERE code = 'BF') "
            "WHERE country_id IS NULL"
        )
